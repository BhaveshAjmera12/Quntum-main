...
const Hp = () => {
  ...
  const fetchHpLaptops = async () => {
    try {
      const response = await axios.get("http://localhost:4000/products");
      const filteredLaptops = response.data.products.filter(
        (product) => product.brand.toLowerCase() === "hp"
      );
      setHpLaptops(filteredLaptops);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-32">
      <h1 className="text-3xl font-bold mb-6 text-center">HP Laptops</h1>
