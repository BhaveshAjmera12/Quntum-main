...
const GamingPage = () => {
  ...
  const fetchGamingLaptops = async () => {
    try {
      const response = await axios.get("http://localhost:4000/products");
      const filteredLaptops = response.data.products.filter(
        (product) => product.category.toLowerCase() === "gaming" || product.category.toLowerCase() === "gaming laptop"
      );
      setGamingLaptops(filteredLaptops);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-32">
      <h1 className="text-3xl font-bold mb-6 text-center">Gaming Laptops</h1>
