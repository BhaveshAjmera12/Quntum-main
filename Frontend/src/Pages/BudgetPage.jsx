...
const BudgetPage = () => {
  const [budgetLaptops, setBudgetLaptops] = useState([]);
  const { token, fetchCart } = useCart();
  const navigate = useNavigate();

  const fetchBudgetLaptops = async () => {
    try {
      const response = await axios.get("http://localhost:4000/products");
      const filteredLaptops = response.data.products.filter(
        (product) => product.price <= 45000
      );
      setBudgetLaptops(filteredLaptops);
    } catch (error) {
      console.error("Error fetching budget laptops:", error);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-32">
      <h1 className="text-3xl font-bold mb-6 text-center">Budget Laptops</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {budgetLaptops.map((product) => (
